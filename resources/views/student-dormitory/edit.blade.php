...
@section('content')
    <div class="container">
        ...
        <div class="card row p-3 shadow-sm shadow-rounded">
            ...
            <form action="{{ route('student-dormitory.update',$student_dormitory->id) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="form-group col-md-6 offset-md-3">
                    <label for="">Student Name </label>
                    <input type="text" value="{{$student_dormitory->student_name??''}}" class="form-control" name="student_name" required>
                </div>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <select name="dormitory_id" class="form-control handle_room" id="dormitory_id" required>
                        ...
                        @foreach ($dormitories as $key => $dormitory)
                            <option @if($dormitory->id == $student_dormitory->dormitory_id) selected @endif value="{{ $dormitory->id ?? '' }}">{{ $dormitory->name ?? '' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <select name="room_type_id" class="form-control handle_room" id="room_type_id" required>
                        ...
                        @foreach ($room_types as $key => $room_type)
                            <option @if($room_type->id == $student_dormitory->room->room_type_id) selected @endif value="{{ $room_type->id ?? '' }}">{{ $room_type->type ?? '' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <select name="room_id" class="form-control" id="room_id" required>
                        <option value="{{$student_dormitory->room_id}}">{{$student_dormitory->room->room_number}}</option>
                ...
                <div class="form-group col-md-6 offset-md-3">
                    ...
                    <select name="status" class="form-control" id="" required>
                        <option value="">Select One</option>
                        <option @if($student_dormitory->status == 1) selected @endif value="1">Active</option>
                        <option @if($student_dormitory->status == 0) selected @endif value="0">Inactive</option>
                    </select>
                </div>
